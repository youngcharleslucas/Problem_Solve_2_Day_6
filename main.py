...
# 1.---------Happy Numbers---------------
"""
Unhappy numbers eventually get into a loop of 89, 145, 42, 20, 4, 16, 37, 58
Thus I can create a while loop that will iterate until one of these numbers are found for an unhappy number
    else: =1, stop for happy number

user input for a number
list of unhappy numbers

1. function to convert base number to a list (bases)
    sum list
    for base in bases
        sum list.append (base)
    return sum list
2. function squaring the bases in list (bases list)
    sum
    for base in bases list
        sum = base **2 + sum

function for happy number (user input, unhappy number list)
    unhappy numbers
    counter = 1
    result of bases = user input
    while result of bases != 1:
        bases = fucntion 1 above (result of bases)
        result of bases = function 2 above (bases)
        counter += 1
            if result of bases in unhappy numbers:
                print unhappy number
    else:
        print (user input is a happy number of "counter" iterations)

"""

user_input = int(input("Do you know any happy numbers? Tell me one and we'll try it!: "))
list_of_unhappy = [89, 145, 42, 20, 4, 16, 37, 58]

def bases_to_list(bases):
    bases_list = []
    bases_str = str(bases)
    for base in bases_str:
        bases_list.append(int(base))
    return bases_list


def sum_of_bases_squared (bases_list):
    sum_bases = 0
    for base in bases_list:
        sum_bases = base**2 + sum_bases
    return sum_bases

def happy_num_nums(user_input, list_of_unhappy):
    unhappy_numbers = list_of_unhappy
    counter = 1
    original_number = user_input
    sum_of_bases = user_input
    while sum_of_bases != 1:
        bases = bases_to_list (sum_of_bases)
        sum_of_bases = sum_of_bases_squared (bases)
        counter += 1
        if sum_of_bases in unhappy_numbers:
            print (f'{original_number} is an unhappy number :(')
            break
    else:
        print(f'{original_number} is a happy number with an iteration of {counter}!!!! :)')
